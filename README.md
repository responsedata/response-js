...
Run `npm logout` afterward if the npm session should not remain available on
the machine. Wrangler can also use a temporary `CLOUDFLARE_API_TOKEN` scoped to
this account with only R2 write access instead of an interactive login.
